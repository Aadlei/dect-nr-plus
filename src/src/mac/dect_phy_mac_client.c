					       ...
					       union nrf_modem_dect_phy_hdr *out_phy_header)
{
	...
	if (header.packet_length < 0) {
		LOG_ERR("(%s): Phy pkt len calculation failed", (__func__));
		return -EINVAL;
	}
	...
	if (total_byte_count <= 0) {
		LOG_ERR("Unsupported slot/mcs combination");
		return -EINVAL;
	}
	...
	if (err) {
		LOG_WRN("(%s): Failed to add padding: err %d (continue)", __func__, err);
	}
...
static void dect_phy_mac_client_rach_tx_worker(struct k_work *work_item)
{
	...
	if (!scan_info) {
		LOG_WRN("(%s): Beacon with long RD ID %u has not been seen in scan results",
	...
	if (err) {
		LOG_ERR("(%s): client_rach_tx failed: %d", (__func__), err);
	}
				...
				struct dect_phy_mac_rach_tx_params *params)
{
	...
	ret = dect_phy_mac_client_data_pdu_encode(params, target_nbr->nw_id_24msb,
						  ...
						  &pdu_ptr, &phy_header);
	if (ret < 0) {
		LOG_ERR("(%s): Failed to encode client data pdu", __func__);
		return ret;
	}
	slot_count = ret + 1;
	...
	if (ra_start_mdm_ticks == 0) {
		LOG_ERR("(%s): Failed to get next RACH TX time", __func__);
		return -EINVAL;
	}
	...
	if (!sched_list_item) {
		LOG_ERR("(%s): dect_phy_api_scheduler_list_item_alloc_tx_element failed: No "
	...
	if (!dect_phy_api_scheduler_list_item_add(sched_list_item)) {
		LOG_ERR("(%s): dect_phy_api_scheduler_list_item_add failed", (__func__));
		dect_phy_api_scheduler_list_item_dealloc(sched_list_item);
		return -EBUSY;
	}
	LOG_INF("Scheduled random access data TX:\n"
	...
	struct dect_phy_mac_associate_params *params)
{
	...
	ret = dect_phy_mac_client_association_req_pdu_encode(
		...
		&pdu_ptr, &phy_header);
	if (ret < 0) {
		LOG_ERR("(%s): Failed to encode a association req", __func__);
		return ret;
	}
	slot_count = ret + 1;
	...
	if (!sched_list_item) {
		LOG_ERR("(%s): dect_phy_api_scheduler_list_item_alloc_rx_element failed",
			   (__func__));
		ret = -ENOMEM;
		goto err_exit;
	}
	...
	if (!dect_phy_api_scheduler_list_item_add(sched_list_item)) {
		LOG_ERR("(%s): dect_phy_api_scheduler_list_item_add for RACH failed",
			   (__func__));
		ret = -EBUSY;
		dect_phy_api_scheduler_list_item_dealloc(sched_list_item);
	}

	LOG_INF("Scheduled random access data TX/RX:\n"
		   "  target long rd id %u (0x%08x), short rd id %u (0x%04x),\n"
		   "  target 32bit nw id %u (0x%08x), tx pwr %d dbm,\n"
		   "  channel %d, payload PDU byte count: %d,\n"
		   "  beacon interval %d, frame time %lld, beacon received %lld",
...
static void dect_phy_mac_client_associate_resp_timeout_worker(struct k_work *work_item)
{
	...
	LOG_WRN("Association response timeout: not associated with device long RD ID %d",
				  ...
				  struct dect_phy_mac_associate_params *params)
{
	...
	if (association_data == NULL) {
		...
		if (association_data == NULL) {
			LOG_ERR("(%s): Max amount of associated clients", (__func__));
			return -EINVAL;
		}
	} else {
		LOG_WRN("(%s): Association exists for target long rd id %u - continue",
	...
	if (err) {
		LOG_ERR("(%s): dect_phy_mac_client_associate_msg_send failed: %d",
			(__func__), err);
		return err;
	}
	...
	dect_phy_mac_association_resp_t *association_resp)
{
	...
	if (!association_data) {
		LOG_WRN("(%s): No association data found for transmitter id %u",
	...
	LOG_INF("(%s): associated with device %d - starting background scan",
		(__func__), common_header->transmitter_id);
	...
	if (dect_phy_mac_nbr_bg_scan_start(&bg_scan_params)) {
		LOG_WRN("(%s): dect_phy_mac_nbr_bg_scan_start failed", (__func__));
	} else {
	...
	struct dect_phy_mac_associate_params *params)
{
	...
	ret = dect_phy_mac_client_association_rel_pdu_encode(
		...
		&pdu_ptr, &phy_header);
	if (ret < 0) {
		LOG_ERR("(%s): Failed to encode association release req", __func__);
		return ret;
	}
	slot_count = ret + 1;
				   ...
				   struct dect_phy_mac_associate_params *params)
{
	...
	if (!association_data) {
		LOG_WRN("(%s): Not associated with device %d",
...
{
	LOG_INF("Client status:");
	for (int i = 0; i < DECT_PHY_MAC_MAX_NEIGBORS; i++) {
		if (client_data.associations[i].state !=
		    DECT_PHY_MAC_CLIENT_ASSOCIATION_STATE_DISASSOCIATED) {
			LOG_INF("  Association #%d: long RD ID %u",
